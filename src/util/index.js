...
const handleCountUrl = () => {
    let user_county = window.localStorage.getItem("user_county");
    let user_info = window.localStorage.getItem('wx_userinfo');
    let {
        openid
    } = JSON.parse(user_info);
    let queryString = qs.stringify(getValidStr(openid));

    let locale = '',
        status = false;
    if (user_county.substr(0, 4) == "2102") {
        // 大连
        locale =
            "http://bank.test.vicgoo.com/index.php/Home/Index/index/type/1";
        locale = locale + '?' + queryString;
        status = true;
    } else if (user_county.substr(0, 4) == "3101") {
        // 上海
        locale =
            "http://bank.test.vicgoo.com/index.php/Home/Index/index/type/2";
        locale = locale + '?' + queryString;
        status = true;
    }
    return {
